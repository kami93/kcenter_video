...
def save_checkpoint(path_to_job, model, optimizer, scaler, epoch, cfg, cur_iter=0, path_to_checkpoint=None):
    """
    Save a checkpoint.
    Args:
        model (model): model to save the weight to the checkpoint.
        optimizer (optim): optimizer to save the historical state.
        scaler (GradScaler): AMP gradscaler to save the scaling state.
...
#        if True:
    ...
    if inflation:
        ...
        missing_keys, unexpected_keys = ms.load_state_dict(pre_train_dict, strict=False)
...
def load_train_checkpoint(cfg, model, optimizer, scaler):
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        if cfg.TRAIN.LOAD_MODEL_STATES_ONLY:
            logger.info("Load only model states from given checkpoint file.")
            load_checkpoint(
                cfg.TRAIN.CHECKPOINT_FILE_PATH,
                model,
                cfg.NUM_GPUS > 1,
            ...
            logger.info("Load all states from given checkpoint file.")
            checkpoint_epoch = load_checkpoint(
                cfg.TRAIN.CHECKPOINT_FILE_PATH,
                model,
                cfg.NUM_GPUS > 1,
