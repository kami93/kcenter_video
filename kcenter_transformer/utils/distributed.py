...
def broadcast(tensors, src=0):
    """
    Broadcast the provided tensors to all processes across machines (from src rank <default: 0>).
    Args:
...
def all_gather_local(tensors):
    """
    All gathers the provided tensors from all processes in _LOCAL_PROCESS_GROUP.
    Args:
        tensors (list): tensors to perform all gather across all processes in _LOCAL_PROCESS_GROUP.
    """
    ...
    for tensor in tensors:
        ...
        dist.all_gather(tensor_placeholder, tensor, group=_LOCAL_PROCESS_GROUP, async_op=False)
        gather_list.append(tensor_placeholder)
    for gathered_tensor in gather_list:
        output_tensor.append(torch.cat(gathered_tensor, dim=0))
    return output_tensor
