...
def load_pretrained(
    ...
    filter_fn=None, strict=True, progress=False):
    ...
    """
    #  Load state dict
    assert (f"{cfg.KCENTER_TIMESFORMER.PRETRAINED_WEIGHTS} not in [vit_1k, vit_1k_large]")
    state_dict = load_state_dict_from_url(url=default_cfgs[cfg.KCENTER_TIMESFORMER.PRETRAINED_WEIGHTS])
