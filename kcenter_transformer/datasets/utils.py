    ...
    for i in range(retry):
        ...
        for image_path in image_paths:
            with PathManager.open(image_path, "rb") as f:
                ...
                img = cv2.imdecode(img_str, flags=cv2.IMREAD_COLOR) # bgr

            imgs.append(img)

        if all(img is not None for img in imgs):
            imgs = torch.as_tensor(np.stack(imgs))
            imgs = reverse_color_channels(imgs, channel_dim=-1) # bgr -> rgb
...
def spatial_sampling(
    ...
    if spatial_idx == -1:
        ...
        frames, _ = transform.random_crop(images=frames, size=crop_size, seed=seed)

        if random_horizontal_flip:
            frames, _ = transform.horizontal_flip(prob=0.5, images=frames, seed=seed)
...
def frames_augmentation(frames, colorjitter=False, use_grayscale=False, use_gaussian=False, seed=None):
    """
    Args:
    frames (tensor): frames of images sampled from the video. The
    ...
    if colorjitter:
        if random_number >= 0.2:
            frames = transform.color_jitter(frames, 0.4, 0.4, 0.4, seed=seed)
...
def reverse_color_channels(frames, channel_dim=-1):
    """
    Reverse the color channels (e.g. bgr -> rgb) for given frames.
    Args:
        frames (tensor or ndarray): 4D array. e.g., T x C x H x W or T x H x W x C
        channel_dim (int): the channel dimension.
    """
    ind = [slice(None)]*4
    ind[channel_dim] = [2, 1, 0]

    frames = frames[tuple(ind)]
